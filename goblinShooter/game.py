...
class Player(object):
    ...
    def hit(self):
        ...
        win.blit(text, ((WIN_W/2 - (text.get_width()/2), WIN_H/2)))
...
class Enemy(object):
    ...
    def __init__(self, x, y, width, height,start, end):
         ...
         self.walkCount = 0
         self.val = 3
         self.hitbox = (self.x + 17, self.y + 2, 40, 57)
         self.health = 10
         self.visible = True
...
goblin = Enemy(100, 410, 64, 64, 50, 760)
shootLoop = 0
bullets = []
run = True
while run:
    ...
    if keys[pygame.K_SPACE] and shootLoop == 0:
        ...
        if len(bullets) < 5:
            if man.left:
                new = Projectile(round((man.x + 10)), round(man.y + man.height//2), 6, (0, 0, 0), facing)
            else:
                new = Projectile((round(man.x + man.width//2 + 20)), round(man.y + man.height//2), 6, (0, 0, 0), facing)
    ...
    if keys[pygame.K_LEFT]:
        ...
        if keys[pygame.K_s]:
            ...
            man.x+=man.val
        if man.x + man.width > WIN_W:
            man.x = WIN_W - man.width
